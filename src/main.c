...
void setupContext( RenderContext *_render_context, int _w, int _h, int _r, int _g, int _b )
{
	// Place the two framebuffers vertically in VRAM.
	SetDefDrawEnv(&(_render_context->buffers[0].draw_env), 0,  0, _w, _h);
	SetDefDispEnv(&(_render_context->buffers[0].disp_env), 0,  0, _w, _h);
	SetDefDrawEnv(&(_render_context->buffers[1].draw_env), 0, _h, _w, _h);
	SetDefDispEnv(&(_render_context->buffers[1].disp_env), 0, _h, _w, _h);

	// Set the default background color and enable auto-clearing.
	setRGB0( &( _render_context->buffers[0].draw_env ), _r, _g, _b );
	setRGB0( &( _render_context->buffers[1].draw_env ), _r, _g, _b );
	_render_context->buffers[0].draw_env.isbg = 1;
	_render_context->buffers[1].draw_env.isbg = 1;

	// Initialize the first buffer and clear its OT so that it can be used for
	// drawing.
	_render_context->active_buffer = 0;
	_render_context->next_packet = _render_context->buffers[0].buffer;
	ClearOTagR(_render_context->buffers[0].ot, OT_LENGTH);

	// Turn on the video output.
	SetDispMask(1);
}

void swapBuffers(RenderContext *_render_context)
{
	...
	RenderBuffer *draw_buffer = &(_render_context->buffers[_render_context->active_buffer]);
	RenderBuffer *disp_buffer = &(_render_context->buffers[_render_context->active_buffer ^ 1]);
	...
	// pointer.
	_render_context->active_buffer ^= 1;
	_render_context->next_packet = disp_buffer->buffer;
	ClearOTagR(disp_buffer->ot, OT_LENGTH);
}

void* createPrimitive(RenderContext *_render_context, int _z, size_t _size)
{
	// Place the primitive after all previously allocated primitives, then
	// insert it into the OT and bump the allocation pointer.
	RenderBuffer *buffer = &(_render_context->buffers[_render_context->active_buffer]);
	uint8_t *prim = _render_context->next_packet;

	addPrim( &( buffer->ot[ _z ] ), prim );
	_render_context->next_packet += _size;

	// Make sure we haven't yet run out of space for future primitives.
	assert(_render_context->next_packet <= &(buffer->buffer[BUFFER_LENGTH]));
...
// by calling FntLoad().
void draw_text(RenderContext *_render_context, int _x, int _y, int _z, const char *_text)
{
	RenderBuffer *buffer = &( _render_context->buffers[ _render_context->active_buffer ] );

	_render_context->next_packet = (uint8_t *)
		FntSort( &( buffer->ot[ _z ] ), _render_context->next_packet, _x, _y, _text );
...
int main(int argc, const char **argv)
{
	...
	ResetGraph( 0 ); /*  */
	FntLoad( 960, 0 ); /* font load */
	
	// Set up our rendering context.
	RenderContext render_context;
	setupContext(&render_context, SCREEN_WIDTH, SCREEN_HEIGHT, 63, 0, 127);
	...
	for (;;)
	{
		...
		if (x < 0 || x > (SCREEN_WIDTH - 64))
			dx = -dx;
		if (y < 0 || y > (SCREEN_HEIGHT - 64))
			dy = -dy;
		...
		TILE *tile_primitive = (TILE *)createPrimitive(&render_context, 1, sizeof(TILE));

		setTile(tile_primitive);
		setXY0(tile_primitive, x, y);
		setWH(tile_primitive, 64, 64);
		setRGB0(tile_primitive, 255, 255, 0);
